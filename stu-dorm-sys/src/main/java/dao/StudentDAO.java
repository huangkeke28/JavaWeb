...
public class StudentDAO {
    ...
    public static List<Student> query(Page p) {
        ...
        try {
            ...
            if (p.getSortOrder() != null && p.getSearchText().trim().length() > 0) {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                int count = rs.getInt("count");//设置到返回数据的total字段,当前字段是无法通过返回du对象设置
    ...
    public static int apply(Student s) {
        ...
        try {
            c = DBUtil.getConnection();
            StringBuffer sql = new StringBuffer("update student set dorm_id=? " +
                    "   where id in (");
