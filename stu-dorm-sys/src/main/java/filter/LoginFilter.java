...
//过滤器满足:http请求的url匹配过滤器路径规则,才会过滤(调用filter种的方法)
@WebFilter("/*")
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        //2.后端服务资源:除登录接口/uer/login以外,其他接口,没有登录,返回没有登录的json信息
        //获取session信息
        ...
        if (session == null) {//没有登录
            ...
            if ("/public/page/main.html".equals(uri)) {//首页没有登录就重定向
                ...
                String contextPath = req.getContextPath();//部署名
                String basePath = schema + "://" + host + ":" + port + contextPath;
                res.sendRedirect(basePath + "/public/index.html");
                return;
            } else if (!"/uer/login".equals(uri) && !uri.startsWith("/public/")
